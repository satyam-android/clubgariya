...
public class CreateGroupViewModel extends AndroidViewModel {
    ...
    private List<TransactionReferenceUser> transactionReferenceUsers;
    ...
    public void createGroup(List<User> users, String groupName, String groupDesc, String profileImage, String calledBy) {
        referenceUsers = new ArrayList<>();
        transactionReferenceUsers = new ArrayList<>();
        ...
        FirebaseObjectHandler.getInstance().getUserCollection().add(groupUser).addOnSuccessListener(new OnSuccessListener<DocumentReference>() {
            ...
            public void onSuccess(DocumentReference documentReference) {
                ...
                if (calledBy.equalsIgnoreCase(AppConstants.USER_LIST_FOR_TRANSACTION))
                    createTransactionReference(groupName, description);
                else
    ...
    private void createTransactionReference(String groupName, String groupDesc) {
        String userROle;
        for (int i = 0; i < users.size(); i++) {
            ...
            transactionReferenceUsers.add(new TransactionReferenceUser(users.get(i).getUid(), userROle,0.0,0.0));
        }
        transactionReference = new TransactionReference(referenceId,CurrentUserData.getInstance().getUid(),groupUser.getUid(), AppConstants.REFERENCE_TYPE_GROUP, System.currentTimeMillis(), groupDesc,0.0,userUids, transactionReferenceUsers);

        FirebaseObjectHandler.getInstance().getTransactionReferenceCollection().document(referenceId).set(transactionReference).addOnSuccessListener(new OnSuccessListener<Void>() {
            @Override
...
//                registerUserForNotification();
    ...
    private void createChatReference(String groupName, String groupDesc) {
        ...
        chatReference = new ChatReference(referenceId, System.currentTimeMillis(), groupDesc, groupName, CurrentUserData.getInstance().getUid(), AppConstants.REFERENCE_TYPE_GROUP, "", referenceUsers, userUids, groupUser.getUid());

        FirebaseObjectHandler.getInstance().getChatReferenceCollection().document(referenceId).set(chatReference).addOnSuccessListener(new OnSuccessListener<Void>() {
            @Override
            public void onSuccess(Void aVoid) {
                Log.e(TAG, "onSuccess: REFRENCE created");
//                registerUserForNotification();
    ...
    private void updateUserGroup(String groupName) {
        List<String> list;
        if(calledBy.equalsIgnoreCase(AppConstants.USER_LIST_FOR_TRANSACTION))
            list = CurrentUserData.getInstance().getTransactionGroup();
        else    list = CurrentUserData.getInstance().getChatGroups();
        if (list==null){
            list = new ArrayList<>();
        }
        list.add(groupName);
        ...
        if(calledBy.equalsIgnoreCase(AppConstants.USER_LIST_FOR_TRANSACTION))
        map.put(AppConstants.USER_TRANSACTION_GROUP_TAG_, list);
      else  map.put(AppConstants.USER_CHAT_GROUP_TAG_, list);
        FirebaseObjectHandler.getInstance().getUserDocumentReference(CurrentUserData.getInstance().getUid()).update(map).addOnSuccessListener(new OnSuccessListener<Void>() {
            @Override
            public void onSuccess(Void aVoid) {
                listner.onGroupCreatedSuccess(chatReference,transactionReference);
            }
        }).addOnFailureListener(new OnFailureListener() {
            @Override
            public void onFailure(@NonNull Exception e) {
    ...
    private void createReferenceId() {
        ...
        if (comapareValue > 0) {
            ...
            referenceId = groupUser.getUid() + FirebaseObjectHandler.getInstance().getFirebaseAuth().getUid();
//            Log.e(TAG, "setupUpChatDatabase: UID greater  " + chatId);
        } else if (comapareValue == 0) {
            referenceId = groupUser.getUid() + groupUser.getUid();
        } else if (comapareValue < 0) {
            //contact UID is greater than user UID
            referenceId = FirebaseObjectHandler.getInstance().getFirebaseAuth().getUid() + groupUser.getUid();
